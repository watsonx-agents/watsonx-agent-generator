...
Choosing any of the framework-based options results in the creation of a dedicated folder (inside `assets/frameworks`) with the structure:

```
assets/frameworks/<framework_name>/model
    ├── agent.py
    └── __init__.py
```

If you select **no framework**, a folder named **base** is created instead, with the same inner structure.

## Agent Customization

After selecting a framework and setting up the base model files, the wizard offers an optional customization step.  
If you choose to customize the agent, you are prompted to provide a new task description.  
The system then:
1. Loads the respective `agent.py` (copied to the framework folder).
2. Invokes the AI generator (implemented in `agent_creator/generator.py`) that takes the original code and your task description as inputs.
3. Generates updated agent code while preserving the overall structure.
4. Writes this updated code into a new file named **agent_custom.py** in the project root.

This mechanism allows you to tailor the agent’s behavior for a specific task while keeping the underlying structure intact.
...
1. **Clone the Repository**
   ...
   ```bash
   git clone https://github.com/watsonx-agents/watsonx-agent-generator
   ```
...
3. **Create a New Agent**
   ...
   - Optionally, you can choose to customize your agent by providing a new task description.  
     If you do, an updated agent code will be generated in **agent_custom.py**.
...
- **Modular Design:** The agent’s core logic is encapsulated in the **model/agent.py** file (or **agent_custom.py** if customized), so developers can easily update or extend the functionality.
